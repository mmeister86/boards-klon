...
export const findDropAreaById = (areas: DropAreaType[], id: string): DropAreaType | null => {
...
export const isDropAreaEmpty = (area: DropAreaType): boolean => {
  ...
  if (area.isSplit && area.splitAreas.some((subArea) => !isDropAreaEmpty(subArea))) return false
  return true
}

// Filter out empty drop areas for preview
export const filterNonEmptyDropAreas = (dropAreas: DropAreaType[]): DropAreaType[] => {
      ...
      (area.isSplit &&
        area.splitAreas.some(
          (subArea) =>
            ...
            (subArea.isSplit && subArea.splitAreas.some((nestedArea) => nestedArea.blocks.length > 0)),
        )),
  )
}
...
export const findParentOfSplitAreas = (
  ...
  for (const area of areas) {
    if (area.isSplit && area.splitAreas.length === 2) {
      const hasFirstArea = area.splitAreas.some(splitArea => splitArea.id === firstAreaId);
      const hasSecondArea = area.splitAreas.some(splitArea => splitArea.id === secondAreaId);
    ...
    if (area.isSplit && area.splitAreas.length > 0) {
      const parent = findParentOfSplitAreas(area.splitAreas, firstAreaId, secondAreaId);
...
export const canMergeAreas = (
  ...
  // Both areas must be empty OR one must be empty and one must have exactly one block
  const validContents = (
    (firstAreaEmpty && secondAreaEmpty) || 
    (firstAreaEmpty && secondArea.blocks.length === 1 && !secondArea.isSplit) || 
    (secondAreaEmpty && firstArea.blocks.length === 1 && !firstArea.isSplit)
  );
  
  if (!validContents) {
    // Debug log: this might be why we can't merge
    console.log(`Cannot merge ${firstAreaId} and ${secondAreaId}: invalid contents`, {
      firstAreaEmpty,
  ...
  if (!canMerge) {
    console.log(`Cannot merge ${firstAreaId} and ${secondAreaId}: not siblings`);
  }
