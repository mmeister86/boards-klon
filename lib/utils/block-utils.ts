...
export const getBlockStyle = (block: BlockType, viewport: ViewportType = "desktop") => {
  const baseStyle = (() => {
    switch (block.type) {
      case "heading": {
        ...
        return headingStyles[headingLevel as keyof typeof headingStyles] || headingStyles[1];
      }
      case "paragraph":
        return "text-base"
      case "image":
