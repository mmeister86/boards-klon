...
export default function MediaCategory({
  ...
  // Context7: Pre-Fetching State für jedes Item
  // Wir speichern für jedes Item, ob es im (erweiterten) Viewport ist
  const [visibleMap, setVisibleMap] = useState<Record<string, boolean>>({});
  const itemRefs = useRef<Record<string, HTMLDivElement | null>>({});

  useEffect(() => {
    // Context7: Intersection Observer für Pre-Fetching
    const observer = new window.IntersectionObserver(
      ...
      { rootMargin: "400px" } // Pre-Fetching: 400px vor dem Viewport
    );
    // Alle Refs beobachten
            ...
            <DialogTrigger asChild>
              ...
              >
                <div className="w-full h-full rounded-lg overflow-hidden flex items-center justify-center shadow-sm shadow-black/30">
                  {/* Context7: Pre-Fetching für image und video */}
                  {type === "video" && (
                    <div className="relative w-full h-full">
                      {visibleMap[item.id] || idx < 8 ? (
                        ...
                        // Platzhalter, solange nicht sichtbar
                        <div className="w-full h-full flex items-center justify-center bg-muted">
