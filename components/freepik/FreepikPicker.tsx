...
export function FreepikPicker({ onSelect, onClose }: FreepikPickerProps) {
  ...
  const searchFreepik = async () => {
    ...
    try {
      const response = await fetch(
        `/api/freepik/search?q=${encodeURIComponent(
          searchTerm
        )}&type=${mediaType}`
      );
    ...
    try {
      ...
      if (data.downloadUrl) {
        ...
        if (data.mediaItem) {
          ...
          console.log(
            "Using fallback without mediaItemId:",
            data.partialSuccess ? "(partial success)" : ""
          );
  ...
  console.log("Current download state:", {
    ...
    itemsWithDownloadingState: results.filter((m) => m.id === isDownloading)
      .length,
  });
  ...
  return (
    <div className="p-4 space-y-4">
      <Tabs
        value={mediaType}
        onValueChange={(v) => setMediaType(v as "photo" | "video")}
      >
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="photo">Bilder</TabsTrigger>
